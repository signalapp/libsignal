...
public class SenderKeyName: ClonableHandleOwner {
    ...
    public init(distributionId: String, senderName: String, deviceId: UInt32) throws {
        var handle: OpaquePointer?
        try checkError(signal_sender_key_name_new(&handle, distributionId, senderName, deviceId))
        super.init(owned: handle!)
    }

    public convenience init(distributionId: String, sender: ProtocolAddress) throws {
        try self.init(distributionId: distributionId, senderName: sender.name, deviceId: sender.deviceId)
    }
    ...
    public var distributionId: String {
        return failOnError {
            try invokeFnReturningString {
                signal_sender_key_name_get_distribution_id($0, nativeHandle)
            }
        }
    }
...
extension SenderKeyName: Hashable {
    public static func == (lhs: SenderKeyName, rhs: SenderKeyName) -> Bool {
        ...
        return lhs.distributionId == rhs.distributionId
    ...
    public func hash(into hasher: inout Hasher) {
        ...
        hasher.combine(self.distributionId)
    }
}
