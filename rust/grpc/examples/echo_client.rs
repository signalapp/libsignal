...
impl signal_grpc::GrpcReplyListener for SimpleListener {
    fn on_reply<'life0,'async_trait>(&'life0 mut self,reply:signal_grpc::GrpcReply,) ->  core::pin::Pin<Box<dyn core::future::Future<Output = Result<()> > +'async_trait> >where 'life0:'async_trait,Self:'async_trait {
        println!("REPLY: {:?}", reply);
        Box::pin(async {
            Ok(())
        })
