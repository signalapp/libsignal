...
pub const PNI_CREDENTIAL_PRESENTATION_RESULT: [u8; zkgroup::PNI_CREDENTIAL_PRESENTATION_LEN] = [
...
fn test_integration_pni() {
    ...
    let presentation = server_public_params.create_pni_credential_presentation(
        randomness,
        group_secret_params,
        pni_credential,
    );
    ...
    //     print!("0x{:02x}, ", b);
    // }
    assert!(PNI_CREDENTIAL_PRESENTATION_RESULT[..] == presentation_bytes[..]);

    // SERVER
    server_secret_params
        .verify_pni_credential_presentation(group_public_params, &presentation)
        .unwrap();
    ...
    let mut pni_credential_presentation_bytes =
        [0u8; zkgroup::common::constants::PNI_CREDENTIAL_PRESENTATION_LEN];
    ...
    pni_credential_presentation_bytes.copy_from_slice(&bincode::serialize(&presentation).unwrap());
