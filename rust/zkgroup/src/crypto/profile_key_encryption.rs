...
impl ProfileKeyEncryptionDomain {
    pub(crate) fn decrypt(
        ...
        let mut retval = CtOption::new(ProfileKeyStruct::partial_default(), Choice::from(0u8));
            ...
            retval = retval.or_else(|| {
                candidates[i].and_then(|profile_key_bytes| {
                    let mut candidate_retval =
                        CtOption::new(ProfileKeyStruct::partial_default(), Choice::from(0u8));
                    for j in 0..8 {
                        candidate_retval = candidate_retval.or_else(|| {
                            ...
                            CtOption::new(ProfileKeyStruct { bytes: pk, M3, M4 }, found)
                        });
                    }
                    candidate_retval
                })
        ...
        if n_found == 1 {
            // We can unwrap because n_found > 0 implies that candidate_retval is Some, which means retval is Some
            Ok(retval.unwrap())
        } else {
