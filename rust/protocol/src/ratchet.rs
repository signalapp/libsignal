...
pub(crate) fn initialize_alice_session_swoosh<R: Rng + CryptoRng>(
    parameters: &AliceSignalProtocolParameters,
    mut csprng: &mut R,
) -> Result<SessionState> {
    let local_identity = parameters.our_identity_key_pair().identity_key();

    // Derive pswoosh key pair
    let sending_ratchet_key_pswoosh = pswoosh::pswoosh_keygen(&A, true);
    ...
    let (sending_chain_root_key, sending_chain_chain_key) = root_key.create_chain(
        parameters.their_ratchet_key(),
        &sending_ratchet_key_pswoosh.private_key,
    )?;
