...
pub async fn do_restore(
    ...
    let restore1 = Restore1::new(
        masked_secret.server_ids.as_ref(),
        password.as_bytes(),
        &mut rng,
    );
    let responses1 = {
...
mod test {
    ...
    async fn do_backup_fails_with_the_first_error() {
        let result = do_backup::<TestEnv>(NotConnectedResults, "", [0; 32], nonzero!(1u32)).await;
        assert_matches!(result, Err(crate::svr3::Error::ConnectionTimedOut));
    }

    #[tokio::test]
    async fn do_restore_fails_with_the_first_error() {
        let result = do_restore(NotConnectedResults, "", OpaqueMaskedShareSet::default()).await;
        assert_matches!(result, Err(crate::svr3::Error::ConnectionTimedOut));
    }
