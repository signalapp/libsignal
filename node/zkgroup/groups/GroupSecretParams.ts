...
export default class GroupSecretParams extends ByteArray {
  ...
  static generateWithRandom(random: Buffer): GroupSecretParams {
    return new GroupSecretParams(NativeImpl.GroupSecretParams_GenerateDeterministic(random));
  }

  static deriveFromMasterKey(groupMasterKey: GroupMasterKey): GroupSecretParams {
    return new GroupSecretParams(NativeImpl.GroupSecretParams_DeriveFromMasterKey(groupMasterKey.getContents()));
  }
  ...
  getMasterKey(): GroupMasterKey {
    return new GroupMasterKey(NativeImpl.GroupSecretParams_GetMasterKey(this.contents));
  }

  getPublicParams(): GroupPublicParams {
    return new GroupPublicParams(NativeImpl.GroupSecretParams_GetPublicParams(this.contents));
  }
