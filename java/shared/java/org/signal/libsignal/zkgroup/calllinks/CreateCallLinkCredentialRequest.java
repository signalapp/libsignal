...
public final class CreateCallLinkCredentialRequest extends ByteArray {
   ...
   */
  public CreateCallLinkCredentialResponse issueCredential(UUID userId, Instant timestamp, GenericServerSecretParams params) {
   ...
   * {@link #issueCredential(UUID, Instant, GenericServerSecretParams)} if the source of randomness
   * doesn't matter.
   *
   * @param userId The ACI of the user who will be creating the call link.
   * @param timestamp Must be a round number of days. Use {@link java.time.Instant#truncatedTo} to
   ...
   */
  public CreateCallLinkCredentialResponse issueCredential(UUID userId, Instant timestamp, GenericServerSecretParams params, SecureRandom secureRandom) {
    byte[] random      = new byte[RANDOM_LENGTH];
    secureRandom.nextBytes(random);

    byte[] newContents = Native.CreateCallLinkCredentialRequest_IssueDeterministic(getInternalContentsForJNI(), userId, timestamp.getEpochSecond(), params.getInternalContentsForJNI(), random);
